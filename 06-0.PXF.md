...
```
sudo su - oracle
cp $PXF_HOME/conf/* $PXF_BASE/conf
cd lib
wget https://download.oracle.com/otn-pub/otn_software/jdbc/23262/ojdbc11.jar
```
#### make oracle jdbc-site.xml
```
tee $PXF_BASE/servers/oracle/jdbc-site.xml <<EOF
...
<configuration>
  ...
  <property>
    <name>jdbc.url</name>
    <value>jdbc:oracle:thin:@10.0.8.192:1521:orcl</value>
  </property>
  <property>
    <name>jdbc.user</name>
    <value>scott</value>
...
=== pxf oracle scott.emp =====
```
DROP TABLE ora_scott_emp;
CREATE EXTERNAL TABLE ora_scott_emp (
    empno     integer,
    ename     varchar(10),
    job       varchar(9),
    mgr       integer,
...
LOCATION ('pxf://SCOTT.EMP?PROFILE=jdbc&SERVER=oracle')
