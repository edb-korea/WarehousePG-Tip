...
-- 1. Create EXTENSION parallel processing
create extension gp_parallel_retrieve_cursor;

-- 2. Create ROLE
CREATE ROLE remote_user WITH LOGIN PASSWORD 'remote_pwd';

-- 3-1. set privilege at table level
GRANT ALL ON TABLE public.employees TO remote_user;
-- 3-2. set privilege at schema level
GRANT ALL ON ALL TABLES IN SCHEMA public TO remote_user;
-- or
...
-- B. Create TEST Schema
CREATE SCHEMA IF NOT EXISTS tpcds;

-- C. Create TEST Table
CREATE TABLE..

-- D. COPY FROM FOREIGN to LOCAL
