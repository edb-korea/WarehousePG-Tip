...
```
-- A. Create Test Source Database
CREATE DATABASE remote_db;
\c remote_db;

-- B. Create Test Tatble 
...
-- 1. (Options) Create EXTENSION parallel processing
--    Greenplum 인 경우만, 설정 필요. 
...
-- 3-1. 스키마 사용 권한
GRANT USAGE ON SCHEMA tpcds TO remote_user;

-- 3-2. 테이블 단위로
GRANT SELECT ON tpcds.call_center TO remote_user;
-- 3-3. 스키마 내 전체 테이블에 한 번에
GRANT SELECT ON ALL TABLES IN SCHEMA tpcds TO remote_user;

-- 4. set privilege at pg_catalog
GRANT SELECT ON TABLE pg_catalog.gp_endpoints TO remote_user;
GRANT SELECT ON TABLE pg_catalog.gp_session_endpoints TO remote_user;

```

#### 로컬(Local) WHPG 서버 FDW 연동 구성
add remote hosts in /etc/hosts at all segment node
```
10.0.x.1 whpg-m 
10.0.x.2 whpg-sm 
10.0.x.3 whpg-s1 
...
```
-- A. Create TEST Destination Database
Create Database fdw_sf_1;
\c fdw_sf_1;

-- 1. Create EXTENSION whpg_fdw
CREATE EXTENSION IF NOT EXISTS whpg_fdw;

-- 2. Create SERVER
DROP SERVER remote_whpg_server;
CREATE SERVER remote_whpg_server FOREIGN DATA WRAPPER whpg_fdw
OPTIONS (host 'whpg-m',port '5555',dbname 'tpcds_sf_1',
mpp_execute 'all segments',num_segments '4',fetch_size '1000');
 
-- 3. Create USER MAPPING
CREATE USER MAPPING FOR current_user SERVER remote_whpg_server
OPTIONS (user 'remote_user', password 'remote_pwd');

-- 4. Import SCHEMA
CREATE SCHEMA tpcds;
CREATE SCHEMA tpcds_fs;
IMPORT FOREIGN SCHEMA tpcds FROM SERVER remote_whpg_server INTO tpcds_fs;

-- B. Create TEST Table
CREATE TABLE..

-- C. COPY FROM FOREIGN to LOCAL
INSERT INTO tpcds.customer
SELECT * FROM tpcds_fs.customer;
```
